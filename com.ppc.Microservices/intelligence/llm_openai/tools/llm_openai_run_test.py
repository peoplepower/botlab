...
class LLMOpenAITestCase(Enum):
    CHAT_COMPLETION = 1
    RESPONSE = 2

    def content(self) -> dict:
        if self == LLMOpenAITestCase.CHAT_COMPLETION:
            return {
        ...
        if self == LLMOpenAITestCase.RESPONSE:
            return {
                "test_case": "response",
                "params": {
                    "model": "gpt-4o-mini-2024-07-18",
...
# Data Stream Address
DATASTREAM_ADDRESS = "llm_openai_run_test"

DATASTREAM_CONTENT = LLMOpenAITestCase.CHAT_COMPLETION.content()
