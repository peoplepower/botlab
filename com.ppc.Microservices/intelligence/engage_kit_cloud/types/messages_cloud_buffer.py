...
class MessagesCloudBuffer:
    ...
    def add_messages_with_priority(
        self, botengine, messages: List[BotMessage], phrases: List[dict]
    ):
        """
        Add messages to the buffer with calculated priority.
        """
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            ">add_messages_with_priority()"
        )
        ...
        for msg in messages:
            if msg.status != CloudMessageStatus.DELIVERED.value:
                botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                    "|add_messages_with_priority() appending message_id={}, status={}".format(
        ...
        for topic_id, msgs in topics.items():
            for msg in msgs:
                botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                    "|add_messages_with_priority() message_id={}, topic_id={}, priority_phrase={}, priority={}".format(
                        msg.message_id,
                        msg.topic_id,
                        msg.priority_phrase(),
                        self.calculate_priority(msg.priority_phrase()),
                    )
                )

            topic_priorities[topic_id] = CloudTopicPriority(
                round(
                    mean(
                        [
                            self.calculate_priority(
                                msg.priority_phrase(), phrases
                            ).value
                            for msg in msgs
                        ]
                    )
        ...
        for topic_id, msgs in topics.items():
            for msg in msgs:
                botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                    "|add_messages_with_priority() adding message_id={}, topic_id={}, priority={}".format(
                        msg.message_id, msg.topic_id, topic_priorities[topic_id]
                    )
                )
    ...
    def get_scheduled_messages_with_adjusted_delivery_times(
        self,
        botengine,
        time_threshold: int,
        ...
        """
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            ">get_scheduled_messages_with_adjusted_delivery_times()"
        )
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            "|get_scheduled_messages_with_adjusted_delivery_times() time_threshold={} midnight_timestamp_s={}".format(
                time_threshold, midnight_timestamp_s
            )
        )
        ...
        for _scheduled_time, _priority, _count, message in self._buffer:
            if message is not self.REMOVED:
                botengine.get_logger(f"{__name__}.{__class__.__name__}").debug(
                    "|get_scheduled_messages_with_adjusted_delivery_times() check message buffer..."
                )
                ...
                botengine.get_logger(f"{__name__}.{__class__.__name__}").debug(
                    "|get_scheduled_messages_with_adjusted_delivery_times() \tlast_delivery_time={}".format(
                        last_delivery_time
                    )
                )
                ...
                if message.status == CloudMessageStatus.READY.value:
                    botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                        "|get_scheduled_messages_with_adjusted_delivery_times() \tScheduling. message={}".format(
                            message
                        )
                    )
                    # If the time is in the past then adjust it based on the current time
                    botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                        "|get_scheduled_messages_with_adjusted_delivery_times() \tcurrent_timestamp_s={}, midnight_timestamp_s={}, delivery_date={}".format(
                            current_timestamp_s, midnight_timestamp_s, delivery_date
                        )
                    )
                    if (
                        current_timestamp_s - midnight_timestamp_s + time_threshold
                        > delivery_date
                    ):
                        delivery_date = (
                            current_timestamp_s - midnight_timestamp_s + time_threshold
                        ...
                        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                            "|get_scheduled_messages_with_adjusted_delivery_times() \tadjusted delivery time based on current time."
                        )

                    # If there's a previous message for the same topic and the time is close, adjust it
                    if last_delivery_time is not None:
                        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                            "|get_scheduled_messages_with_adjusted_delivery_times() \tChecking for adjustments. {} < {}".format(
                                delivery_date - last_delivery_time, time_threshold
                            )
                        )
                        if (delivery_date - last_delivery_time) < time_threshold:
                            delivery_date = min(
                                last_delivery_time + time_threshold,
                                (message.max_delivery_time / 1000)
                                - midnight_timestamp_s,
                            )
                            botengine.get_logger(
                                f"{__name__}.{__class__.__name__}"
                            ).info(
                                "|get_scheduled_messages_with_adjusted_delivery_times() \tadjusted delivery time based on threshold. min({}, {})".format(
                                    last_delivery_time + time_threshold,
                                    (message.max_delivery_time / 1000)
                                    - midnight_timestamp_s,
                                )
                    ...
                    updated_delivery_date_ms = (
                        midnight_timestamp_s + delivery_date
                    ) * 1000
                    botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                        "|get_scheduled_messages_with_adjusted_delivery_times() \tupdated_delivery_date_ms={}".format(
                            updated_delivery_date_ms
                        )
                    )
                ...
                if (
                    last_delivery_time_per_topic.get(message.topic_id, 0)
                    < delivery_date
                ):
                    last_delivery_time_per_topic[message.topic_id] = delivery_date

        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            "<get_scheduled_messages_with_adjusted_delivery_times() list_of_bot_updated_messages={}".format(
                list_of_bot_updated_messages
            )
        )
    ...
    def add_message(self, botengine, message: BotMessage, priority: CloudTopicPriority):
        ...
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            "|add_message() _buffer={} entry={}".format(self._buffer, entry)
        )
    ...
    def get_next_message(self, botengine) -> BotMessage:
        ...
        while self._buffer:
            ...
            if message is not self.REMOVED:
                ...
                botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                    "<get_next_message() message={}".format(message)
                )
                return message
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            "<get_next_message() No message"
        )
        return None

    def get_closest_scheduled_message(self, botengine) -> tuple[int, BotMessage]:
        """
        Returns the scheduled time of the next message in the buffer, or None if buffer is empty.
        """
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            ">get_closest_scheduled_message()"
        )
        for i in range(len(self._buffer)):
            scheduled_time, priority, count, message = self._buffer[i]
            if message is not self.REMOVED:
                botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                    "<get_closest_scheduled_message() scheduled_time={}, message={}".format(
                        scheduled_time, message
                    )
                )
                return scheduled_time, message
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            "<get_closest_scheduled_message() No message"
        )
        return (None, None)
    ...
    def remove_messages_by_ids(self, botengine, message_ids: list):
        ...
        for message_id in message_ids:
            messages_to_remove = [
                message
                for _scheduled_time, _priority, _count, message in self._buffer
                if message.message_id == message_id and message is not self.REMOVED
            ]
    ...
    def remove_messages_by_topic_ids(self, botengine, topic_ids: list):
        ...
        """
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            ">remove_messages_by_topic_ids()"
        )
        if not topic_ids:
            botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                "<remove_messages_by_topic_ids() No topic IDs"
            )
            return
    ...
    def find_message_by_id(self, botengine, message_id: str) -> BotMessage:
        ...
        for _scheduled_time, _priority, _count, message in self._buffer:
            if message is not self.REMOVED and message.message_id == message_id:
                botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                    "<find_message_by_id() message={}".format(message)
                )
                return message
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            "<find_message_by_id() Message not found"
        )
        return None

    def get_priority_of_topic_by_message(
        self, botengine, message: BotMessage
    ) -> CloudTopicPriority:
        """
        Return the priority of messages that belong to the same topic as the given message.
        """
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            ">get_priority_of_topic_by_message()"
        )
        entry = self.entry_finder.get(message)
        if entry is not None:
            botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                "<get_priority_of_topic_by_message() priority={}".format(-entry[1])
            )
        ...
        for _scheduled_time, _priority, _count, message in self._buffer:
            if message is not self.REMOVED and message.topic_id == topic_id:
                ...
                if entry is not None:
                    ...
                    botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
                        "<get_priority_of_topic() priority={}".format(-priority)
                    )
                    return CloudTopicPriority(
                        -priority
                    )  # convert back to positive priority value
        botengine.get_logger(f"{__name__}.{__class__.__name__}").info(
            "<get_priority_of_topic() No message"
        )
        return None
    ...
    def update_message(
        ...
        new_priority: CloudTopicPriority = None,
    ):
        """
        ...
        if old_message is None:
            raise ValueError(
                "Message with ID {} not found".format(new_message.message_id)
            )
