...
# Location state name for the reusable prompts mapping
REUSABLE_PROMPTS_STATE_NAME = "openai_reusable_prompts"

def chat(botengine, location_object, intelligence_id, reference, argument=None, provider_api=None, **kwargs):
    """
    ...
    **LLM Provider Responsibility**: It is the responsibility of the LLM Provider
    microservice (e.g., LocationLLMOpenAIMicroservice) to implement best-effort
...
def update_reusable_prompt(botengine, location_object, prompt_key, prompt_id, version, variables=None):
    """
    Add or update a reusable prompt mapping.

    Microservices call this to register their OpenAI Reusable Prompts with the
    LLM provider. The provider stores the mapping in the
    ``openai_reusable_prompts`` location state (via set_location_property_separately)
    so any microservice can look it up at runtime.

    See https://developers.openai.com/api/docs/guides/prompt-engineering#reusable-prompts

    :param botengine: BotEngine environment
    :param location_object: Location object
    :param prompt_key: Unique key identifying this prompt (e.g., "report_section_content_daily")
    :param prompt_id: OpenAI prompt ID (e.g., "pmpt_abc123")
    :param version: Version string for this prompt (e.g., "1"). Defaults to "current
    :param variables: Optional list of variable names the prompt accepts
    """
    content = {
...
def remove_reusable_prompt(botengine, location_object, prompt_key):
    """
    Remove a reusable prompt mapping.

    :param botengine: BotEngine environment
    :param location_object: Location object
    :param prompt_key: Key of the prompt to remove
    """
    content = {
        "prompt_key": prompt_key,
        "deleted": True,
