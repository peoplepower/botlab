...
datastream_address = "calculate_invoice"
...
def main(argv=None):
    ...
    parser.add_argument("--admin_username", dest="admin_username", help="Admin Username")
    parser.add_argument("--admin_password", dest="admin_password", help="Admin Password")
    ...
    send_datastream_message(server, app_key, datastream_address, datastream_content, args.organization_id)
...
def send_datastream_message(server, app_key, address, content, organization_id):
    ...
    print("Body: " + json.dumps(content, indent=2, sort_keys=True))
    print("Params: " + json.dumps(params, indent=2, sort_keys=True))
    print("Server: " + server)

    r = requests.post(server + "/cloud/appstore/stream", params=params, data=json.dumps(content), headers=http_headers)
    j = json.loads(r.text)
    _check_for_errors(j)
